<?php

use PHPUnit\Framework\TestCase;

class Merchant_HashConnect_Test extends TestCase
{
    public function testMerchant_no3ds()
    {   
        $this->assertSame('pay_ok', $this->executeExchangeCommand(__FUNCTION__, HC['no_3ds']['1']));
    }
    
    public function testMerchant_3ds()
    {   
        $this->assertSame('merchant_purchase_pretransact_ok', $this->executeExchangeCommand(__FUNCTION__, HC['3ds_1']['1']));
    }
    
    
    
	private function init($test_name) 
	{
		$this->loger = new Logger($test_name, 'merchant', LOGS_DIR);
		$this->db = new DBConnect(MYSQL_EXCHANGE_HOST, MYSQL_EXCHANGE_LOGIN, MYSQL_EXCHANGE_PASSWORD, MYSQL_EXCHANGE_DB, $this->loger);
	}

    private function executeExchangeCommand($test_name, $card, $timeout = 60)
    {   
		$this->init($test_name);
		$rows = [
				['w1_site_id_pay',         '1-1'],
				['w1_private_key',         ''],
				['w1_state_url',           'http://192.168.121.5:3110/systems/hash_connect/callback.php'],
				['w1_decline_url',         'http://192.168.121.5:3110/systems/hash_connect/from_bank_to_cabinet.php'],
				['w1_success_url',         'http://192.168.121.5:3110/systems/hash_connect/from_bank_to_cabinet.php'],
				// ['w1_public_key_callback', ''],
				['w1_public_key_callback', ''],
				['service_password',       'zachislyator'],
				['service_crt',            ''],
				['service_key',            ''],
				['service_login',          'zachislyator'],
				['w1_site_id_merchant',    '1-1'],
				['w1_term_url',            'http://192.168.121.5:3110/systems/hash_connect/from_bank_to_cabinet.php'],
				['w1_acs_url',             'http://192.168.121.5:3110/300/test_bank_acs.php'],
				['url_referrer',           'http://192.168.121.5:3110/referrer.php'],
				['url_referrer_3ds2',      'http://192.168.121.5:3110/systems/hash_connect/referrer_3ds2.php'],
				['notification_3ds2',      'http://192.168.121.5:3110/systems/hash_connect/notification_3ds2.php'],
			];
		$how_point = $this->db->createHashconnectGate($test_name, $rows);
        $ext_transact = (string) rand(1000000000000, 9999999999999);
        
        $fields = json_encode(["curr" => "978",
        "method_decline" => "GET",
        "skeys" => "",
        "url_callback" => "",
        "expMonth" => "05",
        "amount" => "0.01",
        "method_callback" => "GET",
        "sid" => "83597",
        "pan" => "$card",
        "is_tokenize" => "HOLDER NAME",
        "url_success" => "https://my-lab.paypoint.pro/proc/fill/ps/show_result/",
        "token_name" => "",
        "client_country_code" => "RU",
        "keyt" => "40903978300100146637",
        "ip" => "192.168.121.5",
        "email" => "some@mail.ru",
        "phone" => "88888888888",
        "cardHolder" => "IVANON IVAN",
        "url_decline" => "https://my-lab.paypoint.pro/proc/fill/ps/show_result/",
        "sender_curr_code" => "RUB",
        "ext_transact" => $ext_transact,
        // "ext_transact" => "1488",
        "cvv" => "123",
        "free_param" => "",
        "request" => "PAY",
        "method_success" => "GET",
        "token" => "",
        "expYear" => "2024"
        ]);
		$transact = $this->db->insertExchange(PROTOCOL_HASHCONNECT, $how_point, 'merchant_purchase_pretransact', $fields);
		
        if (array_search($card, HC['3ds_1'])) {
            sleep(30);
            $selenium = new Selenoid($this->loger);
            $selenium->set_url("http://192.168.121.5:3100/web/merchant/index.php?transact=$transact&test_name=$test_name");
            $selenium->simple_referrer();
            sleep(30);
        }
        sleep($timeout);
        
        $merchant_db = new DBConnect(MYSQL_FORGATE_HOST, MYSQL_FORGATE_LOGIN, MYSQL_FORGATE_PASSWORD, MYSQL_FORGATE_DB, $this->loger);

		$next_operation = $merchant_db->get_merchant_next_operation($ext_transact);
        
		$this->loger->end_log();
		// можно и оставить для истории
		// $this->db->deleteGate($how_point);
		
		return $next_operation;
    }
    

}